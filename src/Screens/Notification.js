...
export default class Notification extends Component{
    ...
    UNSAFE_componentWillReceiveProps(nextProps){
        ...
        if(nextProps.user.getNotificationError && nextProps.user.getNotificationError !== user.getNotificationError){
            Alert.alert(I18n.t('alertWentWrong'))
        }
        if(nextProps.user.getNotification && nextProps.user.getNotification !== user.getNotification){
            if(nextProps.user.getNotification.length > 0){
                this.setState({
                    getNotification:[...getNotification, ...nextProps.user.getNotification],
                    loading:false,
                    page: page+1,
                    refreshing:false,
                })
    ...
    rendergetNotification(){
        ...
        for(let index = 0; index < getNotification.length; index++){
            ...
            results.push(
                <TouchableOpacity style={[styles.branch, element.seen == 0 && {backgroundColor:'#efefef'}]}
                    >  
                    <View style={styles.ListTitleBox}>
                        <AntDesign
                            style={[styles.iconColor, element.seen == 0 && {color:'#ff6b6b'}]}
                            name="checkcircle"
                        >
                        </AntDesign>
                    ...
                    <View style={styles.dateBox}>
                        <Text style={styles.Date}>
                            {element.dateTime && moment(element.dateTime).format("DD-MMMM-YYYY")}
                        </Text>
                    </View>
                </TouchableOpacity>
                        ...
                        <View style={styles.inner}>
                            <View style={styles.btnBack}>
                                <TouchableOpacity onPress={()=>{NavigationService.goBack()}}>
                                    <MaterialIcons
                                        style={{color:'white',marginRight:'20%',fontSize:33}} name="keyboard-arrow-left"> 
                                    </MaterialIcons>
                                </TouchableOpacity>
                            </View>
                            <View style={styles.benner}>
                                <Text style={{fontSize:22, color:'white',fontFamily:'Battambang-Bold'}}>
                                    {I18n.t('notification')}
                                </Text>
                            </View>   
                            <View style={styles.btnBack}>
